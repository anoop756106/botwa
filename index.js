           ...
           return false
			...
			switch(command) {
                ...
                case 'slap':
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					const slap =['poda myre','manushyan alle pulle','bloody rascal','abhi ck kottashion kodukkatte','nirthi podai','poda pathi','buhahahahaha','ayseri idi beno','ookku kolland po myre','i will kell','nen danger aanu mwonu','ivan shalyam anallo','idich montha polickum','anthass venamada anthass','ulupp undo']
          ...
          if (!isRegistered) return reply(ind.noregis())
            await client.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                const trut =['who is abhi to you?','saddest part of your life','your biggest fear?','your first kiss?','your first love?']
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`prefix has been changed to : ${prefix}`)
					break 
				case 'tiktokstalk':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `order recieved ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    client.blockUser (`${body.slice(9)}@c.us`, "remove")
					client.sendMessage(from, `commands recieved ${body.slice(9)}@c.us`, text)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('𝗦successfully changed icon')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'This is ma babe >_<, dont spam him he will kill u',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `your admin is uninstalled🏃 :\n`
							teks += `@_.split('@')[0]`
						}
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `congrats mwonu you are admin chugick (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`congrats @${mentioned[0].split('@')[0]} youre an admin chugick (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
